...
Public Sub Show As String
	...
	If App.ExistInCache(CacheName) = False Then
		App.WriteToCache(CacheName, $Endpoints$Page)
	End If
...
Public Sub Modal (Action As String, CategoryList As List, Data As Map) As String
	Select Action
		Case "Add"
			...
			Dim option1 As MiniHtml = Helper.Option.up(select1)
			option1.attr("value", "")
			option1.text("Select Category")
			option1.selected
			option1.disabled
			For Each row As Map In CategoryList
				Dim option2 As MiniHtml = Helper.Option.up(select1)
...
Private Sub ContainerContent As MiniHtml
	Dim content1 As MiniHtml = Helper.Div.cls("row mt-3")
	Dim col12 As MiniHtml = Helper.Div.up(content1).cls("col-md-12")
	Dim form1 As MiniHtml = Helper.Form.up(col12).cls("form mb-3")
	Dim row1 As MiniHtml = Helper.Div.up(form1).cls("row")
	Dim col1 As MiniHtml = Helper.Div.up(row1).cls("col-md-6 col-lg-6")
	Dim group1 As MiniHtml = Helper.Div.up(col1).cls("input-group mb-3")
	Dim label1 As MiniHtml = Helper.Label.up(group1)
	label1.attr("for", "keyword")
	label1.cls("input-group-text mt-2")
	label1.text("Search")
	Dim input1 As MiniHtml = Helper.Input.up(group1)
	input1.attr("type", "text")
	input1.cls("form-control col-md-6 mt-2")
	input1.attr("id", "keyword")
	input1.attr("name", "keyword")
	Dim searchBtn As MiniHtml = Helper.Button.up(group1)
	...
	Dim col2 As MiniHtml = Helper.Div.up(row1).cls("col-md-6 col-lg-6")
	Dim div2 As MiniHtml = Helper.Div.up(col2).cls("float-end mt-2")
	...
	Helper.Icon.up(button1).cls("bi bi-plus-lg me-2")
	button1.text("Add $Endpoints$")
	Dim container1 As MiniHtml = Helper.Div.up(col12)
...
Private Sub $Endpoints$Table As MiniHtml
	Dim table1 As MiniHtml = Helper.Table
	table1.cls("table table-bordered table-hover rounded small")
	Dim thead1 As MiniHtml = Helper.Thead.cls("table-light").up(table1)
	Helper.Th.up(thead1).sty("text-align: right; width: 50px").text("#")
	Helper.Th.up(thead1).text("Code")
	Helper.Th.up(thead1).text("Name")
	Helper.Th.up(thead1).text("Category")
	Helper.Th.up(thead1).sty("text-align: right").text("Price")
	Helper.Th.up(thead1).sty("text-align: center; width: 120px").text("Actions")
	Helper.Tbody.up(table1)
	Return table1
End Sub

Private Sub $Endpoints$TableRow As MiniHtml
	Dim tr1 As MiniHtml = Helper.Tr
	Helper.Td.up(tr1).cls("align-middle").sty("text-align: right")'.text("{id}")
	Helper.Td.up(tr1).cls("align-middle")'.text("{code}")
	Helper.Td.up(tr1).cls("align-middle")'.text("{name}")
	Helper.Td.up(tr1).cls("align-middle")'.text("{category}")
	Helper.Td.up(tr1).cls("align-middle").sty("text-align: right")'.text("{price}")
	Dim td6 As MiniHtml = Helper.Td.up(tr1)
	td6.cls("align-middle text-center px-1 py-1")
	Dim a1 As MiniHtml = Helper.Anchor.up(td6)
	...
	Helper.Icon.up(a1).cls("bi bi-pencil")
	a1.attr("title", "Edit")
	Dim a2 As MiniHtml = Helper.Anchor.up(td6)
	...
	Helper.Icon.up(a2).cls("bi bi-trash3")
	a2.attr("title", "Delete")
	Return tr1
End Sub

Private Sub ModalAdd As MiniHtml
	Dim form1 As MiniHtml = Helper.Form
	form1.attr("hx-post", "/hx/$endpoints$")
	form1.attr("hx-target", "#modal-messages")
	form1.attr("hx-swap", "innerHTML")
	Dim modalHeader As MiniHtml = Helper.Div.up(form1)
	modalHeader.cls("modal-header")
	Dim h51 As MiniHtml = Helper.H5.up(modalHeader)
	h51.cls("modal-title").text("Add $Endpoint$")
	Dim close1 As MiniHtml = Helper.Button.up(modalHeader)
	close1.attr("type", "button")
	close1.cls("btn-close")
	close1.attr("data-bs-dismiss", "modal")
	Dim modalBody As MiniHtml = Helper.Div.up(form1)
	modalBody.cls("modal-body")
	Helper.Div.up(modalBody).attr("id", "modal-messages")
	Dim group1 As MiniHtml = Helper.Div.up(modalBody)
	group1.cls("form-group")
	Dim label1 As MiniHtml = Helper.Label.up(group1)
	label1.attr("for", "category1")
	label1.text("Category ")
	Dim span1 As MiniHtml = Helper.Span.up(label1)
	span1.cls("text-danger").text("*")
	Dim select1 As MiniHtml = Helper.SelectTag.up(group1)
	select1.cls("form-select")
	select1.attr("id", "category1")
	select1.attr("name", "category")
	select1.required
	Dim group2 As MiniHtml = Helper.Div.up(modalBody)
	group2.cls("form-group")
	Dim label2 As MiniHtml = Helper.Label.up(group2)
	label2.text("Code ")
	Dim span2 As MiniHtml = Helper.Span.up(label2)
	span2.cls("text-danger").text("*")
	Dim input2 As MiniHtml = Helper.Input.up(group2)
	input2.attr("type", "text")
	input2.attr("name", "code")
	input2.cls("form-control")
	input2.required
	Dim group3 As MiniHtml = Helper.Div.up(modalBody)
	group3.cls("form-group")
	Dim label3 As MiniHtml = Helper.Label.up(group3)
	label3.text("Name ")
	Dim span3 As MiniHtml = Helper.Span.up(label3)
	span3.cls("text-danger").text("*")
	Dim input3 As MiniHtml = Helper.Input.up(group3)
	input3.attr("type", "text")
	input3.attr("name", "name")
	input3.cls("form-control")
	input3.required
	Dim group4 As MiniHtml = Helper.Div.up(modalBody)
	group4.cls("form-group")
	Dim label4 As MiniHtml = Helper.Label.up(group4)
	label4.text("Price ")
	Dim input4 As MiniHtml = Helper.Input.up(group4)
	input4.attr("type", "text")
	input4.attr("name", "price")
	input4.cls("form-control")
	Dim modalFooter As MiniHtml = Helper.Div.up(form1).cls("modal-footer")
	Dim button1 As MiniHtml = Helper.Button.up(modalFooter)
	button1.attr("type", "submit")
	button1.cls("btn btn-success px-3")
	button1.text("Create")
	Dim button2 As MiniHtml = Helper.Button.up(modalFooter)
...
Private Sub ModalEdit As MiniHtml
	...
	form1.attr("hx-target", "#modal-messages")
	form1.attr("hx-swap", "innerHTML")
	Dim modalHeader As MiniHtml = Helper.Div.up(form1).cls("modal-header")
	Dim h51 As MiniHtml = Helper.H5.up(modalHeader)
	...
	close1.cls("btn-close")
	close1.attr("data-bs-dismiss", "modal")
	Dim modalBody As MiniHtml = Helper.Div.up(form1).cls("modal-body")
	Dim div1 As MiniHtml = Helper.Div.up(modalBody)
	div1.attr("id", "modal-messages")
	Dim id1 As MiniHtml = Helper.Input.up(modalBody)
...
Private Sub ModalDelete As MiniHtml
	...
	form1.attr("hx-target", "#modal-messages")
	form1.attr("hx-swap", "innerHTML")
	Dim modalHeader As MiniHtml = Helper.Div.cls("modal-header").up(form1)
	Dim h51 As MiniHtml = Helper.H5.up(modalHeader)
	...
	close1.cls("btn-close")
	close1.attr("data-bs-dismiss", "modal")
	Dim modalBody As MiniHtml = Helper.Div.cls("modal-body").up(form1)
	Dim div1 As MiniHtml = Helper.Div.up(modalBody)
	div1.attr("id", "modal-messages")
	Dim id1 As MiniHtml = Helper.Input.up(modalBody)
	id1.attr("type", "hidden")
...
Private Sub GitHubLink As MiniHtml
	Dim div1 As MiniHtml = Helper.Div.cls("text-center mb-3")
	Dim a1 As MiniHtml = Helper.Anchor.up(div1)
	...
	Dim svg1 As MiniHtml = Helper.Svg.up(a1)
	svg1.attr("aria-hidden", "true")
	svg1.attr("width", "24")
	svg1.attr("height", "24")
	svg1.attr("version", "1.1")
	svg1.attr("viewBox", "0 0 16 16")
	Dim path1 As MiniHtml = Helper.Path.up(svg1)
...
Private Sub CategoriesLink As MiniHtml
	Dim li1 As MiniHtml = Helper.Li
	li1.cls("nav-item d-block d-lg-block")
	Dim a1 As MiniHtml = Helper.Anchor.up(li1)
	a1.attr("href", "/categories")
	a1.cls("nav-link float-end")
	a1.text("Categories")
	Dim i1 As MiniHtml = Helper.Icon.up(a1)
